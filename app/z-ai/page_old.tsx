...
export default function ZAiPage() {
  ...
  useEffect(() => {
    ...
    const sessionId = `session_${userEmail}_${
  ...
  const quickActions = [
    {
      ...
      icon: Building2,
      prompt:
        "Analyze the current performance of all companies in my portfolio",
    },
    {
  ...
  const handleDocumentPreview = async (document: any) => {
    ...
    try {
      ...
      if (!filePath) {
        throw new Error(
          "No file path could be constructed from document metadata"
        );
      }
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2">
                                <Badge
                                  variant="secondary"
                                  className={`text-xs ${
                                    doc.relevance_score >= 80
                                      ? "bg-green-100 text-green-800"
                                      : doc.relevance_score >= 60
              ...
              <div className="flex items-center gap-3">
                ...
                {previewPanel.document?.relevance_score && (
                  <Badge
                    variant="secondary"
                    className={`text-xs ${
                      previewPanel.document.relevance_score >= 80
                        ? "bg-green-100 text-green-800"
                        : previewPanel.document.relevance_score >= 60
                        ? "bg-yellow-100 text-yellow-800"
                        : "bg-red-100 text-red-800"
                    }`}
                  >
              ...
              {previewPanel.loading ? (
                ...
                <div className="flex-1 flex flex-col">
                  ...
                  <div className="p-2 border-t border-gray-200 bg-gray-50 text-xs text-gray-500 text-center">
                    PDF rendered in browser • Use browser controls for zoom and
                    navigation
                  </div>
                </div>
              ) : (
