      ...
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" className="flex items-center gap-2 h-auto py-2 px-3">
          ...
          <div className="hidden md:flex flex-col items-start">
            <span className="text-sm font-medium text-gray-900">{auth.name}</span>
            <div className="flex items-center gap-1">
              <span className="text-xs text-gray-500">{currentCompany?.role}</span>
              <Badge className={`text-xs px-1.5 py-0 h-4 ${getRoleBadgeColor(currentCompany?.role || '')}`}>
                {getRoleIcon(currentCompany?.role || '')} {currentCompany?.role?.toUpperCase()}
              </Badge>
            </div>
          </div>
        ...
        <div className="px-2 py-2 bg-gray-50 mx-2 rounded-md">
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium">{currentCompany?.companyName}</p>
              <div className="flex items-center gap-2 mt-1">
                <Badge className={`text-xs ${getRoleBadgeColor(currentCompany?.role || '')}`}>
                  {getRoleIcon(currentCompany?.role || '')} {currentCompany?.role?.toUpperCase()}
                </Badge>
              ...
              .map(company => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <Badge className={`text-xs mt-1 ${getRoleBadgeColor(company.role)}`}>
...
function PermissionIndicator({ role }: { role: string }) {
  ...
  const perm = permissions[role as keyof typeof permissions] || permissions.viewer
